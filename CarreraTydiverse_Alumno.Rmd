...
```{r}
cran %>% summarise(across(c(ip_id,package, country)))
...
```{r}
cran%>%filter(r_version == "3.4.0", country == "ES")
```
...
```{r}
cran%>%arrange(country,desc(r_version), ip_id)
```
...
by_package %>%
  summarise(
    count = n(),                     # Número total de elementos
    unique = n_distinct(ip_id),       # Número de elementos distintos en ip_id
...
top_counts <- cran %>%
  count(package) %>%
  filter(n >= quantile(n, probs = 0.99, na.rm = TRUE))
...
top_counts <- cran %>%
  count(package) %>%
  filter(n >= quantile(n, probs = 0.99, na.rm = TRUE))%>%arrange(desc(n))
...
#cran %>%
#summarise(unique = n_distinct()) %>%
#  filter(unique >= quantile(unique, probs = 0.99, na.rm = TRUE))
...
cran%>%filter(size_mb >= 0.5)%>%arrange(desc(size_mb))
...
#ggplot(cran,mapping = aes(x = ))+geom_bar( )
...
```{r}
students2%>%
  pivot_longer(cols = c(2:5), # Columnas con la informacion
               names_to = c("sex", "class"), # Col por las que queremos cambiarlas
               names_sep = "_", # Separa male_1 en sex = male y class = 1
               values_to = "count") %>% # Muevo sus valore a la fila count
  mutate(sex = recode(sex, "male" = "Male", "female" = "Female"))
...
students3%>%#mutate(name = name, class = full_join(), test = test)
  pivot_longer(cols = starts_with("class"),
               names_to = "class",
               values_to = "grade",
               values_drop_na = TRUE)%>%
  mutate(class = gsub("class", "", class))%>%
  pivot_wider(names_from = test, values_from = grade)#Separa midterm y final en col
